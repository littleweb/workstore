...
function database() {
  return (dbPromise ??= new Promise<IDBDatabase>((resolve, reject) => {
    const request = indexedDB.open("workstore-covers", 1);
...
function sanitizeTitle(value: string): string {
  return value.trim().slice(0, 120) || "未命名封面";
}

export async function createDocument() {
  const now = Date.now();
  const loaded = native
    ? await invoke<LoadedDocument>("create_cover_document")
    : await browserWrite({
...
export async function loadDocument(id: string) {
  ...
  if (!cached || (cached.generation === cached.saved && !cached.saving)) {
    ...
    const loaded = native
      ? await invoke<LoadedDocument>("load_cover_document", { id })
  ...
  c.saving = (async () => {
    try {
      while (c.generation > c.saved) {
        ...
        const result = native
          ? await invoke<LoadedDocument>("save_cover_document", {
              document,
...
registerSyncRefresher(async (paths) => {
  ...
  for (const [id, cached] of cache) {
    if (!paths.includes(`data/app.cover/${id}.cover.json`)) continue;
    if (cached.generation !== cached.saved || cached.saving) continue;
    const generation = cached.generation,
      token = cached.token;
    try {
      const loaded = await invoke<LoadedDocument>("load_cover_document", {
        id,
      });
      if (
        cache.get(id) !== cached ||

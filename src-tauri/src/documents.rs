...
fn validate(doc: &Document) -> Result<()> {
    ...
    if doc.kind != "workstore.document" || doc.schema_version != 1 {
...
impl Store {
    fn document_dir(&self) -> Result<PathBuf> {
        checked_dir(&checked_dir(self.root_path(), "data")?, "app.doc")
    }

    fn document_path(&self, id: &str) -> Result<PathBuf> {
        check_id(id)?;
        Ok(self.document_dir()?.join(format!("{id}.doc.json")))
    }

    pub fn list_documents(&self) -> Result<DocumentList> {
        let mut out = DocumentList {
            documents: vec![],
            warnings: vec![],
        };

        for entry in fs::read_dir(self.document_dir()?).map_err(|e| e.to_string())? {
            ...
            match read_file(&path) {
                Ok((doc, _)) if name == format!("{}.doc.json", doc.info.id) => {
    ...
    pub fn create_document(&self) -> Result<LoadedDocument> {
        let time = now();
        let doc = Document {
            kind: "workstore.document".into(),
            schema_version: 1,
            info: DocumentInfo {
        ...
        let path = self.document_path(&doc.info.id)?;
        if path.exists() {
    ...
    pub fn load_document(&self, id: &str) -> Result<LoadedDocument> {
        let (doc, bytes) = read_file(&self.document_path(id)?)?;
        if doc.info.id != id {

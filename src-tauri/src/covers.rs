...
fn validate(doc: &Document) -> Result<()> {
    ...
    if doc.kind != "workstore.cover" || doc.schema_version != 1 {
...
impl Store {
    fn cover_document_dir(&self) -> Result<PathBuf> {
        checked_dir(&checked_dir(self.root_path(), "data")?, "app.cover")
    }

    fn cover_document_path(&self, id: &str) -> Result<PathBuf> {
        check_id(id)?;
        Ok(self.cover_document_dir()?.join(format!("{id}.cover.json")))
    }

    pub fn list_cover_documents(&self) -> Result<DocumentList> {
        let mut out = DocumentList {
            documents: vec![],
            warnings: vec![],
        };

        for entry in fs::read_dir(self.cover_document_dir()?).map_err(|e| e.to_string())? {
            ...
            let name = path
                ...
                .to_string_lossy()
                .to_string();
            if !name.ends_with(".cover.json") {
                continue;
            }
            match read_file(&path) {
                Ok((doc, _)) if name == format!("{}.cover.json", doc.info.id) => {
    ...
    pub fn create_cover_document(&self) -> Result<LoadedDocument> {
        let time = now();
        let doc = Document {
            kind: "workstore.cover".into(),
            schema_version: 1,
            info: DocumentInfo {
        ...
        let path = self.cover_document_path(&doc.info.id)?;
        if path.exists() {
    ...
    pub fn load_cover_document(&self, id: &str) -> Result<LoadedDocument> {
        let (doc, bytes) = read_file(&self.cover_document_path(id)?)?;
        if doc.info.id != id {
...
mod tests {
    use super::*;
    #[test]
    fn cover_roundtrip_conflict_and_backup() {
        let root = std::env::temp_dir().join(format!("workstore-cover-test-{}", Uuid::new_v4()));
        let store = Store::open(root.join("config"), root.join("workspace")).unwrap();
        let original = store.create_cover_document().unwrap();
        assert_eq!(original.document.info.title, "未命名封面");
        let id = original.document.info.id.clone();
        let mut doc = original.document.clone();
        doc.content = Value::String("{\"cover\":\"<h1>Hello</h1>\"}".into());
        let saved = store.save_cover_document(doc.clone(), original.token.clone()).unwrap();
        assert_eq!(store.load_cover_document(&id).unwrap().document.content, doc.content);
        assert_eq!(store.list_cover_documents().unwrap().documents.len(), 1);
        assert!(store.list_documents().unwrap().documents.is_empty());
        assert!(store.save_cover_document(doc, original.token).is_err());
        assert_eq!(store.load_cover_document(&id).unwrap().token, saved.token);
        assert!(store.root_path().join(".workstore/cover-backups").join(format!("{id}.json")).is_file());
        assert!(store.load_cover_document("../../state").is_err());
        drop(store); fs::remove_dir_all(root).unwrap();
    }
}

#[tauri::command]
pub async fn save_cover_export(path: String, data: String) -> Result<()> {
    use base64::{engine::general_purpose::STANDARD, Engine};
    let path = PathBuf::from(path);
    if !path.is_absolute() || !matches!(path.extension().and_then(|s| s.to_str()), Some("png" | "zip")) {

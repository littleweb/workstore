...
const code = transformSync(
  readFileSync(new URL("../src/covers/store.ts", import.meta.url), "utf8"),
...
function harness(invoke, registerSyncRefresher = () => {}) {
  ...
  vm.runInNewContext(code, {
    ...
    require(id) {
      ...
      if (id.includes("documentLifecycle"))
        return { registerDocumentFlusher: () => {} };
...
test("loading or creating a document cannot change the active document until explicitly activated", async () => {
  const store = harness(async (command, args) =>
    loaded(command === "create_cover_document" ? "new" : args.id),
  );
  ...
  assert.equal(
    store.currentDocument("b").lastOpenedAt,
    1,
    "switches must not reorder recent documents",
  );
});
...
test("a slow read cannot roll back content or its token even if a newer edit has already saved", async () => {
  ...
  const saves = [];
  const store = harness(async (command, args) => {
    if (command === "load_cover_document")
      return ++reads === 1 ? loaded("a") : read.promise;
    saves.push(args);
...
test("out-of-order reads cannot replace a newer cache entry installed by another request", async () => {
  ...
  let reads = 0;
  const store = harness(async () =>
    ++reads === 1 ? slow.promise : loaded("b", "newer", 2),
  );
...
test("AI content starts a new editor session and persists through normal guarded saves", async () => {
  let disk = loaded("a", "old content");
  const tokens = [];
  const store = harness(async (command, args) => {
    if (command === "load_cover_document") return structuredClone(disk);
    if (command === "save_cover_document") {
...
test("a sync refresh cannot roll back an edit saved while its read was pending", async () => {
  let refresh;
  let reads = 0;
  const read = deferred(),
    started = deferred();
  const store = harness(
    async (command, args) => {
      if (command === "list_cover_documents")
        return { documents: [loaded("a").document], warnings: [] };
      if (command === "save_cover_document")
        return loaded("a", args.document.content, 2);
      if (++reads === 1) return loaded("a");
      started.resolve();
      return read.promise;
    },
    (fn) => {
  ...
  await store.openDocument("a");
  const syncing = refresh(["data/app.cover/a.cover.json"]);
  await started.promise;
  store.stageDocument("a", { content: "new saved cover" });
  await store.flushDocuments();
  read.resolve(loaded("a", "old remote snapshot"));
  await syncing;
  assert.equal(store.currentDocument("a").content, "new saved cover");
  assert.equal(store.currentDocument("a").revision, 2);
});
